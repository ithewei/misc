...
int db_exec_with_result(HDB hdb, const char* sql, RecordList* records) {
    ...
    DBRecord record;
    for (int r = 1; r <= row; ++r) {  // note: row[0] is th
        for (int c = 0; c < col; ++c) {
...
int dbtable_select(HDB hdb, const char* table, const char* col_names, const char* where, RecordList* records, const char* limit) {
    if (!table || !records) return SQL_ERR;
...
int dbtable_insert(HDB hdb, const char* table, const char* col_names, const char* col_values) {
    if (!table || !col_values) return SQL_ERR;

    char sql[1024];
    if (col_names && strlen(col_names) > 0) {
        snprintf(sql, sizeof(sql), "insert into %s (%s) values (%s);", table, col_names, col_values);
    } else {
        snprintf(sql, sizeof(sql), "insert into %s values (%s);", table, col_values);
    }
...
int dbtable_update(HDB hdb, const char* table, const char* changes, const char* where) {
    if (!table || !changes) return SQL_ERR;

    char sql[1024];
    if (where && strlen(where) > 0) {
        snprintf(sql, sizeof(sql), "update %s set %s where %s;", table, changes, where);
    } else {
        snprintf(sql, sizeof(sql), "update %s set %s;", table, changes);
    }
    return db_exec(hdb, sql);
}
